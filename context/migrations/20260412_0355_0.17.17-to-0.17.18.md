...
**Status:** pending
**aibox CLI:** v0.17.17 → v0.17.18
**processkit:** v0.13.0

## Summary

aibox has been updated from v0.17.17 to v0.17.18. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Review the release notes for each version in this upgrade:
- https://github.com/projectious-work/aibox/releases/tag/v0.17.18

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.17.18 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.17.18` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.17.18"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.17.18.
