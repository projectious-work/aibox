...
| BACK-041 | Backlog structure: separate active from archive for efficient reads | todo | should | Analyse how to reorganize BACKLOG.md so done/archived items don't need to be read every time. Options: separate archive file, collapsed sections, or split into BACKLOG.md (active only) + BACKLOG-ARCHIVE.md (history). Goal: agents and humans only load active items by default, but can investigate past when needed |
