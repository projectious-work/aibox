...
```bash
./scripts/maintain.sh release 0.3.1
```
...
```bash
./scripts/build-macos.sh 0.3.1
```

Attach additional binaries to the release after creation:

```bash
gh release upload v0.3.1 dist/dev-box-v0.3.1-x86_64-apple-darwin.tar.gz
```
...
gh release create v0.3.1 \
  --title "dev-box v0.3.1" \
  --notes-file dist/RELEASE-NOTES.md \
  dist/dev-box-v0.3.1-*.tar.gz
```
...
```bash
./scripts/maintain.sh push-images 0.3.1
```

This pushes both versioned tags (`python-v0.3.1`) and `latest` tags (`python-latest`)
for each flavor.
...
- [ ] `podman pull ghcr.io/projectious-work/dev-box:base-v0.3.1` succeeds
...
```bash
podman build -t ghcr.io/projectious-work/dev-box:python-v0.3.1 images/python/
```
